...
class OverlayService : Service() {
    ...
    override fun onCreate() {
        ...
        startForeground(NOTIFICATION_ID, NotificationCompat.Builder(this, CHANNEL)
            ...
            .setContentText("플로팅 버튼을 눌러 시작/중지")
    ...
    private fun scheduleNext() {
        handler.removeCallbacksAndMessages(null)
        if (!running) return
        val delay=15_000L+random.nextInt(15_001)
        handler.postDelayed({ performSwipe() },delay)
    }
    ...
    private fun performSwipe() {
        ...
        scope.launch {
            ...
            withContext(Dispatchers.Main) {
                if (result.isFailure) setRunning(false) else scheduleNext()
            }
        }
    }
    ...
    private inner class FloatingButtonView: View(this@OverlayService) {
        ...
        private var downRawX=0f; private var downRawY=0f; private var downX=0; private var downY=0; private var moved=false
        init { setLayerType(LAYER_TYPE_SOFTWARE,null) }
        override fun onDraw(c:Canvas) {
            super.onDraw(c)
            val cx=width/2f; val cy=height/2f; val radius=minOf(width,height)*0.43f
            ...
            paint.style=Paint.Style.FILL; c.drawCircle(cx,cy,radius,paint); paint.clearShadowLayer()
            paint.style=Paint.Style.STROKE; paint.strokeWidth=dp(2).toFloat(); paint.color=Color.rgb(34,211,167); c.drawCircle(cx,cy,radius-dp(1),paint)
            paint.style=Paint.Style.FILL; paint.color=Color.WHITE
            if(running){
                val bw=dp(5).toFloat(); val bh=dp(20).toFloat(); val gap=dp(5).toFloat()
                ...
                val p=Path(); p.moveTo(cx-dp(7),cy-dp(12)); p.lineTo(cx+dp(14),cy); p.lineTo(cx-dp(7),cy+dp(12)); p.close(); c.drawPath(p,paint)
            }
        ...
        override fun onTouchEvent(e:MotionEvent):Boolean {
            when(e.actionMasked){
                MotionEvent.ACTION_DOWN->{downRawX=e.rawX;downRawY=e.rawY;downX=params.x;downY=params.y;moved=false;animate().scaleX(.94f).scaleY(.94f).setDuration(70).start();return true}
                MotionEvent.ACTION_MOVE->{val dx=Math.round(e.rawX-downRawX);val dy=Math.round(e.rawY-downRawY);if(kotlin.math.abs(dx)>dp(5)||kotlin.math.abs(dy)>dp(5))moved=true;params.x=downX+dx;params.y=downY+dy;try{windowManager.updateViewLayout(this,params)}catch(_:Throwable){};return true}
                MotionEvent.ACTION_UP,MotionEvent.ACTION_CANCEL->{animate().scaleX(1f).scaleY(1f).setDuration(90).start();prefs.edit().putInt("x",params.x).putInt("y",params.y).apply();if(!moved&&e.actionMasked==MotionEvent.ACTION_UP)toggle();return true}
            }
            return super.onTouchEvent(e)
        }
    }

    companion object { private const val CHANNEL="autoswipe_runtime"; private const val NOTIFICATION_ID=4301 }
}
